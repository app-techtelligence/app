...
- **Lighthouse mobile performance budget miss.** Home measures 79–88 against the spec's ≥ 90 target. In CPU-comparable paired runs (`benchmarkIndex` 730 vs 779), production scored 91 and this branch 84. Branch JS bootup is 1339 ms vs 787 ms and main-thread work 3721 ms vs 1755 ms; the largest new trace entry is the Home page chunk at 702 ms for a 3.9 KB file — that is execution cost, not download. The cause is `ContourField` compiling its shader and running its rAF loop under 4× CPU throttling. Fonts are not the cause: `font-display` scores a perfect 1, no font audit appears among the Lighthouse opportunities, and the LCP element is the Archivo `<h1>`. The §4 pre-committed remedy (dropping IBM Plex Mono) is therefore inapplicable — it would cost a typeface and recover none of the points. Deferring shader-program creation off the hydration path is an open optimization; it has not yet been attempted.
